...
function updateHp( id, damage ) {
	...
	if ( damage === 0 ) {
		...
		hpUpdateMsg.textContent = 'Nothing happened.';
	}
	...
	if ( damage < 0 ) {
		...
		// Display message.
		hpUpdateMsg.textContent = `${ charName } was healed for ${ ( hpCurrent -hpLast ) } points!`;
	}
...
// Remove the Add New and Start the Battlep buttons, but only if we actually have some characters.
...
//Build the initiative list. Order by initiative first, then by Dex score. Adds an input for recording damage and a button to save the damage input.
